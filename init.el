...
;; TODO 'yank' links in 'org-mode' if what is currently in the 'kill-ring' looks like a link
...
(global-set-key (kbd "C-c C-l") (lambda () (interactive) (insert "lambda ")))
(global-set-key (kbd "C-M-m") 'kmacro-end-and-call-macro)
(global-set-key (kbd "C-M-S-m") 'kmacro-start-macro)  ; or use C-M-M, but not both
(global-set-key (kbd "C-x C-i") 'package-install)
...
;; TODO This should be done before like in `early-init.el'
...
;; Bind keys for when the region is active
...
(insert-or "j" 'laluxx/duplicate-region-replace)
...
;; TODO C-k should kill until the next ')' if there is ' befre the cursor
...
;; TODO add a variable colled pop-height and use it for all of them
;; so we can hardcode only the one we want to chaneeg the haight from the default
...
(defun laluxx/create-haskell-logo-frame ()
  ...
  (let* ((image-path (expand-file-name "images/haskell.png" user-emacs-directory))
...
;;                                     face-lines)))
;;          (new-content "")
;;          (org-loaded (featurep 'org)))

;;     ;; Temporarily load org-mode if it's not already loaded
;;     (unless org-loaded
;;       (require 'org))
...
;;     ;; Unload org-mode if it wasn't initially loaded
;;     (unless org-loaded
...
(global-set-key (kbd "j") (lambda () (interactive)
                            (if (use-region-p)
                                (call-interactively 'laluxx/generate-face-colors)
                              (insert "j"))))

(global-set-key (kbd "G") (lambda () (interactive)
                            (if (use-region-p)
                                (call-interactively 'generate-face-definitions)
                              (insert "G"))))
...
(use-package persistent-scratch
  :load-path "~/.config/emacs/lisp/persistent-scratch"
  :config
  (persistent-scratch-mode 1)
  (setq persistent-scratch-autosave-interval 60)  ; Save every 60 seconds
  (setq persistent-scratch-file-name "persistent-scratch.el")
  (setq persistent-scratch-load-on-scratch-open t)
  :bind
  ("C-c s" . persistent-scratch-open))
...
(use-package gclone
  :load-path "~/.config/emacs/lisp/gclone"
  :config
  (setq gclone-progress-bar-smoothness 'quarter-blocks)
...
(use-package dired-image-preview
  ...
  :custom
  (dired-image-preview-scale 1.0)                    ; Smaller preview size
